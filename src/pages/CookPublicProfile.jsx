...
const CookPublicProfile = ({cook}) => {
    ...
    const getUserData = async (id) => {
        try {
            const data = await fetch(`process.env.REACT_APP_SERVER_BASE_URL/users/id`);
    ...
    <div className="h-full bg-gray-200 p-8">
        ...
        <div className="bg-white rounded-lg shadow-xl pb-8">
            ...
            <div className="w-full h-[250px]">
                {user && user.cover && <img src={user.cover} className="w-full object-cover rounded-tl-lg rounded-tr-lg"/>}
            </div>
            <div className="flex flex-col items-center -mt-20">
                {user && user.avatar && <img src={user.avatar} className="w-40 border-4 border-white rounded-full"/>}
                <div className="flex items-center space-x-2 mt-2">
                    {user && <p className="text-2xl">
        ...
        <div className="my-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
                ...
                <div className="flex-1 bg-white rounded-lg shadow-xl p-8">
                    ...
                    <ul className="mt-2 text-gray-700">
                        <li className="flex border-y py-2">
                            ...
                            {user && <span className="text-gray-700">{`${user.firstName} ${user.lastName}`}</span>}
                        </li>
                        <li className="flex border-b py-2">
                            <span className="font-bold w-24">Data di nascita:</span>
                            {user && user.birthDate && <span className="text-gray-700">{user.birthDate}</span>}
                        </li>
                        <li className="flex border-b py-2">
                            <span className="font-bold w-24">Email:</span>
                            {user && user.email && <span className="text-gray-700">{user.email}</span>}
                        </li>
                        <li className="flex border-b py-2">
                            <span className="font-bold w-24">Contatti:</span>
                            {user && user.phones && user.map((phone) => {
                                return (
                                    <span key={nanoid()} className="text-gray-700 r-2">{phone}</span>
                                )
                            })}
                        </li>
                        <li className="flex border-b py-2">
                            <span className="font-bold w-24">Città coperte:</span>
                            {cook && cook.towns && cook.towns.map((town) => {
                                return (
                                    <span key={nanoid()} className="text-gray-700 r-2">{town}</span>
                                )
                            })}
                        </li>
                        <li className="flex border-b py-2">
                            <span className="font-bold w-24">Link personali:</span>
                            {cook && cook.personalLinks && cook.personalLinks.map((link) => {
                                return (
                                    <a key={nanoid()} href={link.linkSource} className="text-gray-700 r-2">{link.linkName}</a>
                                )
                            })}
                        </li>
                    </ul>
            ...
            <div className="flex flex-col w-full 2xl:w-2/3">
                <div className="flex-1 bg-white rounded-lg shadow-xl p-8">
                    ...
                    {cook && cook.description && <p className="mt-2 text-gray-700">{cook.description}</p>}
                </div>
                <div className="flex-1 bg-white rounded-lg shadow-xl mt-4 p-8">
                    <h4 className="text-xl text-gray-900 font-bold">Menu</h4>
